  ...
  allTopics.forEach((item, index) => {
    ...
    if (is11th) {
      ...
      rawSteps.push({
        ...
        deadlineDate: new Date(today.getTime() + weekIndex * 7 * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
        status: "pending",
        source: "diagnostic",
        autoCompleted: false
      });
...
Верни ответ СТРОГО в формате JSON без markdown-разметки:
{
  ...
  try {
    ...
    const validatedSteps = (result.studyPlan || []).map((step, idx) => ({
      ...
      deadlineDate: step.deadlineDate || new Date(today.getTime() + (idx + 1) * 3 * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
      status: "pending",
      source: "diagnostic",
      autoCompleted: false
    }));
