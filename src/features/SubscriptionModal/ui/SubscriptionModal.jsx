...
export const SubscriptionModal = ({ user, studentStats, onClose }) => {
  ...
  const baseTariffs = {
    basic: { monthly: 1990, name: "Basic" },
    premium: { monthly: 4990, name: "Pro" },
    ultimate: { monthly: 9990, name: "Ultra" },
  };

  const getPriceDisplay = (planId) => {
    const tariff = baseTariffs[planId];
    if (!tariff) return "";

    if (billingCycle === "monthly") {
      return `${tariff.monthly.toLocaleString("ru-RU")} ₸ / мес`;
    } else {
      const seasonalPrice = calculateEntPrice(tariff.monthly, monthsLeft);
      return `${seasonalPrice.toLocaleString("ru-RU")} ₸`;
    }
  };

  const getSavingsDisplay = (planId) => {
    const tariff = baseTariffs[planId];
    if (!tariff || billingCycle !== "seasonal") return 0;

    const totalWithoutDiscount = tariff.monthly * monthsLeft;
    const finalPrice = calculateEntPrice(tariff.monthly, monthsLeft);
    return totalWithoutDiscount - finalPrice;
  };
  ...
  const handleCardPaymentSubmit = async (e) => {
    ...
    if (
      !paymentForm.number ||
      paymentForm.number.replace(/\s/g, "").length !== 16
    ) {
  ...
  const handleRemoveWhitelistEmail = async (emailToRemove) => {
    if (
      !confirm(
        `Вы действительно хотите аннулировать доступ для ${emailToRemove}?`,
      )
    )
      return;
    try {
    ...
    <div className="fixed inset-0 bg-slate-950/50 backdrop-blur-md z-[100] flex items-center justify-center p-4">
      <div className="bg-slate-900 w-full max-w-4xl rounded-[32px] p-8 shadow-2xl border border-slate-800/80 text-white relative max-h-[92vh] overflow-y-auto font-sans custom-scrollbar">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-855 pb-6">
          ...
          {monthsLeft > 2 && (
            <div className="flex items-center gap-1.5 bg-slate-950/60 p-1.5 rounded-2xl border border-slate-800/60 self-start md:self-auto shadow-inner">
              ...
              >
                <span>До ЕНТ ({monthsLeft} мес.)</span>
                <span className="text-[8px] bg-emerald-500 text-white px-1.5 py-0.5 rounded-md font-extrabold uppercase animate-pulse">
                  -{monthsLeft >= 6 ? "25%" : "15%"}
                </span>
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5 pt-6">
          <div
            className={`p-5 rounded-3xl border flex flex-col justify-between transition-all duration-300 relative bg-slate-950/20 ${
          ...
          >
            <div className="space-y-4">
              <div>
                <span className="text-[9px] bg-slate-800 text-slate-400 px-2 py-0.5 rounded-md font-bold uppercase tracking-wider">
                  Free
                </span>
                ...
                <button
                  onClick={async () => {
                    if (studentStats?.role === "founder") {
                      alert(
                        "Как CEO вы имеете полный доступ ко всем функциям.",
                      );
                      return;
                    }
                    if (
                      confirm(
                        "Вы действительно хотите перейти на Бесплатный тариф? Произойдет блокировка платных разделов.",
                      )
                    ) {
                      await updateDoc(doc(db, "users", user.uid), {
                        tariff: "free",
          ...
          >
            <div className="space-y-4">
              <div>
                <span className="text-[9px] bg-indigo-950/40 text-indigo-400 px-2 py-0.5 rounded-md font-bold uppercase tracking-wider border border-indigo-900/20">
                  Пользоваться
                </span>
                <h4 className="text-base font-black text-white mt-1.5">
                  Basic
            ...
            <div className="pt-6 border-t border-slate-800/60 mt-4 space-y-3">
              <div className="text-left">
                ...
                {billingCycle === "seasonal" &&
                  getSavingsDisplay("basic") > 0 && (
                    <p className="text-[9px] text-emerald-400 font-bold mt-0.5">
                      Выгода:{" "}
                      {getSavingsDisplay("basic").toLocaleString("ru-RU")} ₸
                    </p>
                  )}
              </div>
              {studentStats?.tariff === "basic" ? (
          ...
          <div
            className={`p-5 rounded-3xl border flex flex-col justify-between transition-all duration-300 relative bg-gradient-to-b from-indigo-950/20 to-slate-950/10 ${
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                <span className="text-[9px] bg-emerald-950/40 text-emerald-400 px-2 py-0.5 rounded-md font-bold uppercase tracking-wider border border-emerald-900/20">
                  Результат
                </span>
            ...
            <div className="pt-6 border-t border-slate-800/60 mt-4 space-y-3">
              <div className="text-left">
                ...
                {billingCycle === "seasonal" &&
                  getSavingsDisplay("premium") > 0 && (
                    <p className="text-[9px] text-emerald-400 font-bold mt-0.5">
                      Выгода:{" "}
                      {getSavingsDisplay("premium").toLocaleString("ru-RU")} ₸
                    </p>
                  )}
              </div>
              {studentStats?.tariff === "premium" ||
            ...
            <div className="pt-6 border-t border-slate-800/60 mt-4 space-y-3">
              <div className="text-left">
                ...
                <p className="text-lg font-black text-indigo-400 mt-0.5">
                  {getPriceDisplay("ultimate")}
                </p>
                {billingCycle === "seasonal" &&
                  getSavingsDisplay("ultimate") > 0 && (
                    <p className="text-[9px] text-emerald-400 font-bold mt-0.5">
                      Выгода:{" "}
                      {getSavingsDisplay("ultimate").toLocaleString("ru-RU")} ₸
                    </p>
                  )}
              </div>
              {studentStats?.tariff === "ultimate" ||
                ...
                <button
                  onClick={() => {
                    setSelectedPlanForPay({
                      id: "ultimate",
                      name: "Тариф Ultra",
                      price: getPriceDisplay("ultimate"),
                    });
                    setIsPaymentOpen(true);
                  }}
                  className="w-full py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition cursor-pointer"
        ...
        <p className="text-[9px] text-slate-500 italic mt-3 text-center leading-normal">
          * На тарифах Pro и Ultra действует софт-кап дневной активности (300 и
          500 задач соответственно) для защиты от злоупотреблений и багов.
        </p>
          ...
          <div className="flex justify-between items-center">
            ...
            {billingCycle === "seasonal" && (
              <span className="text-[10px] text-emerald-400 font-bold bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                Выгода оптового пакета: до 25% экономии
              </span>
            )}
          </div>
          <div className="border border-slate-800 rounded-2xl overflow-hidden divide-y divide-slate-800 bg-slate-950/20 font-sans text-xs">
            ...
            <div className="grid grid-cols-5 p-3.5 items-center">
              <div className="col-span-2 font-bold text-slate-300">
                ...
                <p className="text-[9px] text-slate-500 font-medium mt-0.5">
                  Доступ ко всем школьным темам
                </p>
              </div>
              <div className="text-center text-slate-400 font-medium">
                1 предмет
              </div>
              <div className="text-center text-slate-200 font-semibold">
                ✓ Да
              </div>
              <div className="text-center text-slate-200 font-semibold">
                ✓ Да
            ...
            <div className="grid grid-cols-5 p-3.5 items-center">
              <div className="col-span-2 font-bold text-slate-300">
                Дневной лимит задач
                <p className="text-[9px] text-slate-500 font-medium mt-0.5">
                  Объем практики кодификатора ЕНТ
                </p>
              </div>
              ...
              <div className="text-center text-indigo-400 font-black">
                500 (Безлимит*)
              </div>
            </div>
            ...
            <div className="grid grid-cols-5 p-3.5 items-center">
              <div className="col-span-2 font-bold text-slate-300">
                ...
                <p className="text-[9px] text-slate-500 font-medium mt-0.5">
                  Автоматический выбор тем для закрытия пробелов
                </p>
              </div>
              <div className="text-center text-slate-500">❌ Нет</div>
              <div className="text-center text-slate-500">
                ❌ Нет
                <p className="text-[8px] text-slate-500/80 leading-none mt-0.5">
                  просто доступ
                </p>
              </div>
              <div className="text-center text-emerald-400 font-bold">
                🌟 Да
                <p className="text-[8px] text-emerald-500/80 leading-none mt-0.5">
                  ведёт по слабым
                </p>
              </div>
              <div className="text-center text-indigo-400 font-bold">
                🌟 Да
                <p className="text-[8px] text-indigo-500/80 leading-none mt-0.5">
                  полное ведение
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-5 p-3.5 items-center">
              <div className="col-span-2 font-bold text-slate-300">
                Умный календарь
                <p className="text-[9px] text-slate-500 font-medium mt-0.5">
                  Автоматическое адаптивное расписание
                </p>
              </div>
              <div className="text-center text-slate-500">❌ Нет</div>
              <div className="text-center text-slate-500">❌ Нет</div>
            ...
            <div className="grid grid-cols-5 p-3.5 items-center">
              <div className="col-span-2 font-bold text-slate-300">
                ...
                <p className="text-[9px] text-slate-500 font-medium mt-0.5">
                  Симуляции и расчет вероятности сдачи
                </p>
              </div>
              <div className="text-center text-slate-500">❌ Нет</div>
              <div className="text-center text-slate-500">❌ Нет</div>
            ...
            <div className="grid grid-cols-5 p-3.5 items-center">
              <div className="col-span-2 font-bold text-slate-300">
                ...
                <p className="text-[9px] text-slate-500 font-medium mt-0.5">
                  Приоритетная offline-генерация вариантов
                </p>
              </div>
              <div className="text-center text-slate-500">❌ Нет</div>
              <div className="text-center text-slate-500">❌ Нет</div>
        ...
        {(studentStats?.role === "founder" ||
          user?.email?.toLowerCase() === "daniilivakin30@gmail.com") && (
          <div className="border-t border-slate-800 pt-8 mt-8 space-y-4">
            ...
            <p className="text-[10px] text-slate-400 leading-normal">
              Добавьте email адреса ваших друзей, чтобы выдать им пожизненный
              бесплатный доступ к Premium-тарифам. Это безопасно и валидируется
              на сервере.
            </p>
          ...
          <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl text-slate-800 relative space-y-4">
            ...
            <div className="bg-slate-50 p-3.5 rounded-2xl flex justify-between items-center border border-slate-100">
              <span className="text-xs text-slate-500 font-medium">
                К оплате ({billingCycle === "monthly" ? "ежемесячно" : "пакет"}
                ):
              </span>
